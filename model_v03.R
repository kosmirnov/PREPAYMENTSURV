...
FNMA$ROE.CAT <- cut(FNMA$ROE, breaks=c(-Inf,-10,10, Inf), labels=c("OTM","ATM","ITM"))
FNMA <- within(FNMA, ROE.CAT <- relevel(ROE.CAT, ref = 2))



FNMA$OPTIONBI <- cut(FNMA$OPTION.LAG1, breaks=c(-Inf,0, Inf), labels=c("OTM","ITM"))

FNMA$LOANAGE.CAT <- cut(FNMA$Loan.Age, breaks=c(-Inf,12,48, Inf), labels=c("< 12 MONTHS","12 - 48 MONTHS","> 48 MONTHS"))

FNMA <- na.omit(FNMA)


#here I used a binary OPTION (incentive) variable to hold for the PH assumption
FINAL<-coxph(Surv(START,STOP,PREPAID)~OPTIONCAT+ROE.SIGN+strata(CSCORE.CAT)+DTI+UNEMP+GDP+PROP_TYP+OCC_STAT+SEASON.Q+MSAREG+YEAR+LOANAGE.CAT, data=FNMA)
summary(FINAL)
extractAIC(FINAL)
ass<- cox.zph(FINAL)

test <- coxph(Surv(START,STOP,PREPAID)~1,FNMA)
extractAIC(test)



#here I used a binary OPTION (incentive) variable to hold for the PH assumption. I stratified for FICOs.
final<-coxph(Surv(START,STOP,PREPAID)~OPTIONBI+ROE.CAT+CSCORE.CAT+DTI+UNEMP+GDP+PROP_TYP+OCC_STAT+SEASON.Q+MSAREG+YEAR+LOANAGE.CAT, data=FNMA)
summary(final)
extractAIC(final)
ass<- cox.zph(final)
