...
FNMA$VOL.CAT <- cut(FNMA$ORIG_AMT, breaks=c(-Inf,350000, Inf), labels=c("1","2"))
...
# SMM plot 
ggplot(SMM.null, aes(x=Month, y=SMM)) +
  geom_line(aes(),colour="#86BC25",size=1)+
  theme_bw()+
  scale_x_continuous(breaks = seq(0, 65, by = 10))+
  scale_y_continuous(breaks = seq(0, 0.04, by = 0.005),labels = scales::percent)
...
#Survival Curve plot
ggsurv.NULL <- ggsurvplot(fit.null, risk.table = TRUE,  palette=c("#86BC25"), censor=FALSE, conf.int = TRUE, xlab="Month", ggtheme=theme_bw(), legend=c("none"),break.x.by = 10,break.y.by = 0.1)
ggsurv.NULL
...
#SMM plot
ggplot(SMM.YIELD, aes(x=time, y=SMM, group=Strata)) +
  geom_line(aes(color=Strata))+
  theme_bw()
ggsave(file = "SMM_YIELD.jpeg")
#CPR plot
ggplot(SMM.YIELD, aes(x=time, y=CPR, group=Strata)) +
  geom_line(aes(color=Strata))+
  theme_bw()
...
SMM.VOL<- SMM(fit.VOL)
# SMM plot
ggplot(SMM.VOL, aes(x=time, y=SMM, group=Strata)) +
  geom_line(aes(color=Strata))+
  theme_bw()
ggsave(file = "SMM_vol.jpeg")
#CPR plot
...
#Survival Curve plot
ggsurv.VOL <- ggsurvplot(fit.VOL, risk.table = TRUE,  palette=c("#86BC25","#0076A8","#BBBCBC"), 
           censor=FALSE, conf.int = TRUE, xlab="Month", ggtheme=theme_bw(),
...
#SMM plot
ggplot(SMM.LTV, aes(x=time, y=SMM, group=Strata)) +
  geom_line(aes(color=Strata))+
  theme_bw()
ggsave(file = "SMM_LTV.jpeg")
#CPR plot
ggplot(SMM.LTV, aes(x=time, y=CPR, group=Strata)) +
  geom_line(aes(color=Strata))+
  theme_bw()
