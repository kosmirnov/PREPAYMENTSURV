...
FINAL_cont<-coxph(Surv(START,STOP,PREPAID)~OPTION+CSCORE+LTV+OCC_STAT+DTI+UNEMP+test+YEAR, data=FNMA)
summary(FINAL_cont)
...
FINAL_disc<-coxph(Surv(START,STOP,PREPAID)~OPTION.BI+LTV.CAT+VOL.CAT+CSCORE+DTI+UNEMP+YEAR, data=FNMA)
summary(FINAL_disc)
zph_disc <- cox.zph(FINAL_disc)
zph_disc
## burn out##
ggcoxzph(zph_disc,var=c("OPTION.BIITM","LTV.CAT>80"),  resid=FALSE,se=TRUE)
